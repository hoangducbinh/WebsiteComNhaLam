    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult Payment(FormCollection f)
        {
            ...
            else
            {
                ...
                {
                    Order ctac = db.Orders.FirstOrDefault(n => n.AccountID == ac.AccountBuyID && n.Date == DateTime.Now);
                    if (ctac == null)
                    {
                        Order od = new Order();
                    ...
                    Order ct = db.Orders.OrderByDescending(m => m.Date).FirstOrDefault(n => n.AccountID == ac.AccountBuyID);
                    foreach (var item in em)
                    {
                        ...
                        Product pr = db.Products.FirstOrDefault(m => m.ProductID == item.ProductID);
                        noidung += "  <tr>";
                        ...
                        db.SaveChanges();
                        CartDetail cd = db.CartDetails.FirstOrDefault(n => n.ProductID == ctdh.ProductID);
                    ...
                    new MailHelper().SendMail(f["email"], "Đơn hàng mới từ Cơm nhà làm", content);
